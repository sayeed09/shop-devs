...
import FooterNudge from '../../components/productv2/footer-nudge';
import { addLimeChat } from '../../utils/productv2/provider';
import { PHWProductId } from '../../utils/product/constants';
...
const MobileFooter = (props: MobileFooterModal) => {
  ...
  useEffect(() => {
    if (props.productId == PHWProductId)
      addLimeChat()
  }, [])
  ...
  const minimumPrice = props.productDetail?.variants.reduce((prev, current) => {
    return prev.price / (30 * prev.position) < current.price / (30 * current.position) ? prev : current;
  });
