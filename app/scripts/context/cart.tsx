...
const reducer = (state: ICartState, action: ICartAction): ICartState => {
  switch (action.type) {
    ...
    case 'SET_SELECTED_FREEBIES':
      return { ...state, selectedFreebies: action.payload };
    case 'SET_COMPARE_AT_PRICES':
      return { ...state, compareAtPrices: action.payload };
    case 'SET_CASH_APPLIED':
      return { ...state, cashApplied: action.payload };
    case 'SET_DISCOUNT_CODE':
      return { ...state, discountCode: action.payload };
    case 'SET_DISCOUNT_CASH_RESPONSE':
      return { ...state, discountAndCashResponse: action.payload };
    case 'SET_INITIAL_CART_LOADING':
      return { ...state, initialCartLoading: action.payload };
    case 'SET_QUICK_BUY_CART':
      return { ...state, quickBuyCart: action.payload };
    case 'SET_DEFAULT_ADDRESS':
      return { ...state, defaultAddress: action.payload };
    case 'SET_EDIT_DEFAULT_ADDRESS':
      return { ...state, editDefaultAddress: action.payload };
    case 'SET_PROCEED_TO_CHECKOUT_EXP':
      return { ...state, ProceedToCheckout: action.payload };
    case 'SET_SHOW_UPGRADE_CART_OPTION':
      return { ...state, showUpgradeCartOption: action.payload };
...
export const Provider = ({ children }: any) => {
  ...
  // Save cart to localStorage when it changes (browser only)
  useEffect(() => {
    if (!isBrowser) return;
    if (state?.localCartItems && !initialFetch) {
      localStorage.setItem('localCartItems', JSON.stringify(state.localCartItems));
    }
  }, [state?.localCartItems, isBrowser, initialFetch]);
